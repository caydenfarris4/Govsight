...
- Enhanced reporting with comprehensive balance sheet analysis

ENHANCED REPORTING FEATURES:
- Executive summaries with AI-generated insights
- Asset vs liability analysis charts
- Balance sheet ratio calculations
...
def render_balance_sheet_position(org: str = "cityA", org_display_name: str = "City A"):
    ...
    with col1:
        ...
        # Create summary bar chart with proper number formatting (no truncation)
        fig = px.bar(
            summary_by_type,
            x='Category',
            y='Actual',
            ...
            color='Category',
            text=[f'${val:,.0f}' for val in summary_by_type['Actual']], # Custom formatting to prevent truncation
            color_discrete_map={
                'Asset': '#0066cc',
                'Liability': '#cc0000',
                'Fund Balance': '#009900',
        ...
        st.plotly_chart(fig, use_container_width=True, key="balance_sheet_summary")
        ...
        st.plotly_chart(fig2, use_container_width=True, key="balance_sheet_by_fund")
...
def load_balance_sheet_data(db_path):
    ...
    """
    try:
        # First check if AccountType column exists in DepartmentPerformance table
        import sqlite3
        ...
        has_account_type = 'AccountType' in columns
        
        if has_account_type:
            # Query with AccountType column (preferred method)
            query = """
            SELECT 
                AccountCode, 
                AccountName, 
            ...
            WHERE 
                AccountType = 'Balance'
            GROUP BY
                AccountCode, AccountName, AccountType, Fund
            """
        else:
            # Fallback query without AccountType - identify balance sheet accounts by pattern
            query = """
            SELECT 
                AccountCode, 
                AccountName, 
                'Balance' as AccountType,
                Fund,
                Fund as FundName,
                SUM(Actual) as Actual
            FROM 
                DepartmentPerformance
            WHERE 
                (AccountCode LIKE '1%' OR AccountCode LIKE '2%' OR 
                 AccountName LIKE '%asset%' OR AccountName LIKE '%liability%' OR 
                 AccountName LIKE '%fund balance%' OR AccountName LIKE '%net position%')
            GROUP BY
