...
from modules.database.db_connection import check_password
import tempfile
...
def generate_pdf_report(title, narrative, df_summary, output_chart=None):
    ...
    # Output as bytes with proper encoding handling
    try:
...
def generate_full_report(df, report_title="GovSight Auto-Generated Report", scenario_context=None):
    ...
    # Also display basic report information
    st.info("Report generated successfully with AI-powered analysis and visualizations.")
...
def add_scenario_to_comparison(scenario_data):
    ...
    if isinstance(scenario_data, dict) and 'id' in scenario_data:
        ...
        if scenario_id not in scenario_ids:
            ...
            from modules.database.connection_manager import get_scenarios  # Import here to avoid circular imports
...
def compare_scenarios_and_generate_report():
    ...
    from modules.database.connection_manager import get_detailed_scenario_data, execute_query  # Import here to avoid circular imports
...
def run_report_generator():
    ...
    # Check user permissions with enhanced feedback
    try:
        if not sec.is_admin() and not sec.is_finance_director():
            st.warning("You don't have permissions to generate financial reports.")
            st.info("Report generation requires Administrator or Finance Director privileges.")
    ...
    with report_tab:
        ...
        if not df.empty:
            ...
            if st.button("Generate Standard Report"):
                try:
                    ...
                    st.info("Please check that the data source contains valid financial data and try again.")
        else:
        ...
        if submit:
            try:
                # Fix the bond/bonds_needed variable issue by ensuring proper variable names
                bonds_needed = max(0, cost - (tax_increase + grant_amount))  # Ensure non-negative
                
                col1, col2 = st.columns([2, 1])
                with col1:
                    st.subheader("Funding Breakdown")
                    remaining_need = bonds_needed  # Use consistent variable naming
                    
                    # Create simple chart for preview
                    fig, ax = plt.subplots(figsize=(6, 4))
                    labels = ['Grant Funding', 'Tax Increase', 'Bonds/Remaining Need']
                    values = [grant_amount, tax_increase, bonds_needed]
                    colors = ['#4CAF50', '#2196F3', '#FF5722']
                ...
                try:
                    ...
                    st.info("Report generation failed. Please check the input values and try again.")
            except Exception as e:
                st.error(f"Error in enhanced report generation: {str(e)}")
                st.info("Please verify all input values are valid and try again.")
