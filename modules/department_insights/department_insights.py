...
- Enhanced reporting with comprehensive visualizations

ENHANCED REPORTING FEATURES:
- Executive summaries with AI-generated insights
- Budget vs actual performance charts
- Spending trend analysis with forecasting
- Department comparison visualizations
- Efficiency metrics and anomaly detection
- Professional PDF reports with embedded Plotly charts
"""
...
from modules.department_insights.department_transaction_integration import display_department_transaction_analysis
...
try:
    ...
    from modules.vatica.govsight_gl_drilldown_module import load_data as load_gl_data
except ImportError:
...
try:
    from modules.ai_hub.ai_hub import ask_ai, generate_dept_insights, generate_forecast
    ...
    if has_real_data:
        ...
        try:
            if selected_dept == "All Departments":
                ...
                st.plotly_chart(fig, use_container_width=True, key="dept_pie_chart")
                ...
                st.plotly_chart(fig2, use_container_width=True, key="dept_bar_chart")
                ...
                fig.update_traces(textposition="inside", textinfo="percent+label")
                st.plotly_chart(fig, use_container_width=True, key=f"fund_pie_chart_{selected_dept}")
                ...
                st.plotly_chart(fig2, use_container_width=True, key=f"fund_bar_chart_{selected_dept}")
                ...
                with st.expander("View Transaction Details and GL Account Balances"):
                    st.markdown(f"""
        ...
        try:
            ...
            if selected_dept == "All Departments":
                ...
                st.plotly_chart(fig3, use_container_width=True, key=f"fiscal_trends_{selected_dept}")
                ...
                if "Month" in filtered_df.columns:
                    ...
                    month_data["Variance"] = month_data["Budget"] - month_data["Actual"]
                    month_data["Variance%"] = (month_data["Variance"] / month_data["Budget"]) * 100
                    ...
                    fig4.update_layout(
                        title=f"{selected_dept} - Monthly Budget vs Actual",
                        xaxis_title="Month",
                        yaxis_title="Amount ($)",
                        barmode="group"
                    )
                    
                    st.plotly_chart(fig4, use_container_width=True, key=f"monthly_bar_{selected_dept}")
        
        except Exception as e:
            st.error(f"Error in department analysis: {e}")
        
        # Enhanced Department Report Generation
        st.markdown("---")
        st.subheader("Enhanced Department Report")
        st.markdown("Generate comprehensive department analysis with executive summary, performance metrics, and visualizations.")
        
        if st.button("Generate Enhanced Department Report", use_container_width=True):
            with st.spinner("Generating comprehensive department report..."):
                try:
                    ...
                    if 'Budget' in report_data.columns and 'Actual' in report_data.columns:
                        report_data['Variance'] = report_data['Budget'] - report_data['Actual']
                        report_data['VariancePercent'] = (report_data['Variance'] / report_data['Budget']) * 100
                        report_data['UtilizationRate'] = (report_data['Actual'] / report_data['Budget']) * 100
                    
                    # Generate enhanced report
                    dept_title = f"Department Performance Analysis: {selected_dept}" if selected_dept != "All Departments" else "All Departments Performance Analysis"
                    enhanced_report = generate_enhanced_report(
                        report_data, 
                        "department", 
                        dept_title,
                        org_display_name,
                        {'fiscal_years': selected_years, 'department': selected_dept}
                    )
                        ...
                        for i, (metric, value) in enumerate(enhanced_report['key_metrics'].items()):
                            with metrics_cols[i % 4]:
                                if isinstance(value, (int, float)):
                                    ...
                                    elif 'amount' in metric.lower() or 'total' in metric.lower() or 'budget' in metric.lower():
                    ...
                    if enhanced_report['charts']:
                        st.markdown("### Department Performance Visualizations")
                        for i, chart in enumerate(enhanced_report['charts']):
                            st.plotly_chart(chart['figure'], use_container_width=True, key=f"enhanced_report_chart_{i}_{selected_dept}")
                    ...
                    if enhanced_report['pdf_bytes']:
                        st.download_button(
                            "Download Complete Department Report (PDF)",
                            data=enhanced_report['pdf_bytes'],
                            file_name=f"department_report_{selected_dept.replace(' ', '_').lower()}_{'-'.join(map(str, selected_years))}.pdf",
                            mime="application/pdf"
                        )
                        
                except ImportError:
                    st.error("Enhanced reporting module not available. Please ensure all dependencies are installed.")
                except Exception as e:
        ...
        if "Month" in filtered_df.columns:
            # Group by month
                    ...
                    st.plotly_chart(fig4, use_container_width=True, key=f"monthly_spending_{selected_dept}")
            ...
            try:
                ...
                if gl_data is not None and not gl_data.empty:
                    ...
                    if 'AccountType' in gl_data.columns:
                        ...
                        st.plotly_chart(fig_gl, use_container_width=True, key=f"gl_accounts_{selected_dept}_{account_type}")
                            ...
                            if 'AccountName' in filtered_gl_data.columns:
                                ...
                                if 'FiscalYear' in forecast_df.columns and len(forecast_df) > 0:
                                    ...
                                    if not forecast_data.empty:
                                        ...
                                        st.plotly_chart(fig, use_container_width=True, key=f"forecast_chart_{selected_account}_{selected_dept}")
                            ...
                            if is_new_schema:
                                ...
                                if not gl_df.empty:
                                    ...
                                    if not operating_df.empty:
                                        ...
                                        fig_gl.update_layout(xaxis_tickangle=-45)
                                        st.plotly_chart(fig_gl, use_container_width=True, key=f"operating_gl_chart_{selected_dept}")
                                ...
                                if has_gl_accounts:
                                    ...
                                    if not gl_df.empty:
                                        ...
                                        fig_gl.update_layout(xaxis_tickangle=-45)
                                        st.plotly_chart(fig_gl, use_container_width=True, key=f"fallback_gl_chart_{selected_dept}")
        ...
        if "Month" in filtered_df.columns and selected_dept != "All Departments":
            ...
            try:
                ...
                st.plotly_chart(fig5, use_container_width=True, key=f"monthly_heatmap_{selected_dept}")
